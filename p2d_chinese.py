...
if args.add_html:
	copyfile(PACKAGE_DIR+'/statements/.html/chinese/problem.html',OUTPUT_DIR+'problem.html')
	copyfile(PACKAGE_DIR+'/statements/.html/chinese/problem-statement.css',OUTPUT_DIR+'problem-statement.css')
...
statements = os.listdir(PACKAGE_DIR + '/statements/.pdf/chinese')
assert len(statements) == 1 #there should be exactly one chinese pdf
for statement in statements:
    copyfile(PACKAGE_DIR + '/statements/.pdf/chinese/' + statement, OUTPUT_DIR + '/' + statement)

#ZIP the OUTPUT and DELETE Temp
package_name = problem_name
package_name = os.path.splitext(os.path.basename(args.package))[0].split('$')[0]

make_archive(OUTPUT_PATH + '/' + package_name + "-chinese", 'zip', OUTPUT_DIR)
rmtree(PACKAGE_DIR)
if nodelete == False:
	rmtree(OUTPUT_DIR)
